...
def extract_scr_features(eda_signal, sample_rate=32):
    """Ekstraksi fitur SCR: jumlah peaks dan amplitudo rata-rata peaks."""
    # Deteksi peaks sebagai respon SCR
...
def extract_scl_feature(eda_signal):
    """Ekstraksi fitur SCL sebagai rata-rata sinyal EDA (level konduktansi kulit)."""
    scl = np.mean(eda_signal)
    return scl

def extract_eda_features(eda_signal, sample_rate=32):
    """Menggabungkan fitur SCR dan SCL."""
